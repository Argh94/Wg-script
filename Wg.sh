    ...
    if curl -fsSL https://raw.githubusercontent.com/Argh94/Wg-script/main/Wg.sh -o ~/.argh94/argh94_warp.sh; then
        ...
        if [ -f ~/.argh94/argh94_warp.sh ] && [ -x ~/.argh94/argh94_warp.sh ]; then
            ...
            if source ~/.bashrc 2>/dev/null; then
                ...
                echo -e "${YELLOW}Warning: Could not reload .bashrc automatically. Please run 'source ~/.bashrc' or restart Termux to use 'Arg'.${RESET}"
            fi
            # Verify alias
            if alias Arg >/dev/null 2>&1; then
                echo -e "${GREEN}Alias 'Arg' is now active!${RESET}"
            else
                echo -e "${YELLOW}Alias 'Arg' is not active yet. Run 'source ~/.bashrc' or restart Termux.${RESET}"
            fi
        else
            echo -e "${RED}Error: Script file is missing or not executable at ~/.argh94/argh94_warp.sh.${RESET}"
            exit 1
        fi
    else
...
echo -e "${CYAN}To use: Copy the config to your VPN app (e.g., WireGuard, V2RayNG, Hiddify) or scan the WireGuard URL.${RESET}"
