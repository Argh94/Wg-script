...
install_script() {
    ...
    if curl -fsSL https://raw.githubusercontent.com/Argh94/Wg-script/main/Wg.sh -o ~/.argh94/argh94_warp.sh; then
        ...
        touch ~/.bashrc
        # Remove any existing Arg alias to avoid duplicates
        ...
        if [ -f ~/.argh94/argh94_warp.sh ] && [ -x ~/.argh94/argh94_warp.sh ]; then
            # Try to source .bashrc in the current session
            if source ~/.bashrc 2>/dev/null; then
                ...
                echo -e "${YELLOW}Warning: Could not reload .bashrc automatically. Please run 'source ~/.bashrc' or restart Termux to use the 'Arg' command.${RESET}"
            fi
            # Verify alias is active
            if alias Arg >/dev/null 2>&1; then
                echo -e "${GREEN}Alias 'Arg' is now active!${RESET}"
            else
                echo -e "${YELLOW}Alias 'Arg' is not active in this session. Please run 'source ~/.bashrc' or restart Termux.${RESET}"
            fi
            # Provide additional instructions for Termux
            echo -e "${CYAN}Note: If 'Arg' does not work, try restarting Termux or running 'source ~/.bashrc' manually.${RESET}"
        else
...
if ! [[ "$new_mtu" =~ ^[0-9]+$ ]] || [ "$new_mtu" -lt 576 ] || [ "$new_mtu" -gt 1500 ]; then
    echo -e "${RED}Error: MTU must be a number between 576 and 1500. Using default 1280.${RESET}"
    new_mtu="1280"
else
...
output_file="$output_dir/warp_config_$(date +%F_%H-%M-%S).txt"
{
    ...
    echo '    "rules": ['
    echo '      {"ip": ["1.1.1.1"], "outboundTag": "proxy", "port": "53", "type": "field"},'
    echo '      {"domain": ["domain:ir", "geosite:category-ir", "geosite:private"], "outboundTag": "direct", "type": "field"},'
    echo '      {"ip": ["geoip:ir", "geoip:private"], "outboundTag": "direct", "type": "field"},'
    echo '      {"domain": ["geosite:category-porn"], "outboundTag": "block", "type": "field"},'
    echo '      {"ip": ["10.10.34.34", "10.10.34.35", "10.10.34.36"], "outboundTag": "block", "type": "field"}'
    echo '    ]'
    echo '  }'
    echo "}"
    echo -e "\nWireGuard URL:"
...
echo -e "${CYAN}To use: Copy the config to your VPN app (e.g., WireGuard, V2RayNG, Hiddify) or scan the WireGuard URL.${RESE
