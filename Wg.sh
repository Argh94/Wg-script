...
echo -e "${GREEN}├──────────────────────────────────┤${RESET}"
echo -e "${GREEN}│${RESET} 1) Generate IPv4 Config          ${GREEN}│${RESET}"
echo -e "${GREEN}│${RESET} 2) Generate IPv6 Config          ${GREEN}│${RESET}"
echo -e "${GREEN}│${RESET} 3) Install Script Permanently    ${GREEN}│${RESET}"
...
output_file="warp_config_$(date +%F_%H-%M-%S).txt"
{
    ...
    echo '  ],'
    echo '  "policy": {'
    echo '    "levels": {"8": {"connIdle": 300, "downlinkOnly": 1, "handshake": 4, "uplinkOnly": 1}},'
    echo '    "system": {"statsOutboundUplink": true, "statsOutboundDownlink": true}'
    echo '  },'
    echo '  "routing": {'
    echo '    "domainStrategy": "IPIfNonMatch",'
    echo '    "rules": ['
    echo '      {"inboundTag": ["dns-in"], "outboundTag": "dns-out", "type": "field"},'
    echo '      {"ip": ["8.8.8.8"], "outboundTag": "direct", "port": "53", "type": "field"},'
    echo '      {"domain": ["geosite:category-ir", "domain:.ir"], "outboundTag": "direct", "type": "field"},'
    echo '      {"ip": ["geoip:ir", "geoip:private"], "outboundTag": "direct", "type": "field"},'
    echo '      {"domain": ["geosite:category-ads-all", "geosite:category-ads-ir"], "outboundTag": "block", "type": "field"},'
    echo '      {"outboundTag": "warp-out", "type": "field", "network": "tcp,udp"}'
    echo '    ]'
    echo '  },'
    echo '  "stats": {}'
    echo "}"
